...
class VideoSerializer(serializers.ModelSerializer):
    """
      ...
      - Validates that either 'url' or 'embed_code' must be present.
      - Auto‐sets 'published_at' when 'is_published=True' and no timestamp given.
      - Marks 'slug', 'created_at', 'updated_at' as read‐only.
      - Provides 'thumbnail_url' (absolute URL) instead of raw file path.
    """
    ...
    # 1. Hyperlinked detail URL field; expects a route named "video-detail" using 'slug' lookup
    url = serializers.HyperlinkedIdentityField(
        view_name="video-detail",   # Adjust if your URLconf uses a different name/namespace
        lookup_field="slug",
        read_only=True,
    )

    # 2. Represent `author` as a string (username or __str__); read‐only
    author = serializers.StringRelatedField(read_only=True)

    # 3. Absolute URL for the thumbnail image (if present)
    thumbnail_url = serializers.SerializerMethodField(read_only=True)

    # 4. Slug is auto‐generated by the model; clients cannot write it
    slug = serializers.ReadOnlyField()

    # 5. created_at / updated_at are auto‐managed; mark read‐only
    created_at = serializers.DateTimeField(read_only=True)
    updated_at = serializers.DateTimeField(read_only=True)

    # 6. published_at is optional, but if is_published=True and none provided, auto‐set in validate()
    published_at = serializers.DateTimeField(required=False, allow_null=True)
    ...
    class Meta:
        model = Video
        fields = [
            "url",              # read‐only hyperlinked identity
            "id",               # primary key
            "slug",             # read‐only
            ...
            "title",
            "description",
            "url",              # note: rename model field? Use 'video_url' if conflicting
            "embed_code",
            "thumbnail",        # allow file upload
            "thumbnail_url",    # read‐only absolute URL
            "duration",
            "status",
            "is_published",
            "published_at",
    ...
    def validate(self, data):
        """
        - Ensure either 'url' or 'embed_code' is present.
        - If 'is_published'=True and 'published_at' missing, set it to now.
        """
        video_url = data.get("url", None)      # model field is named 'url'
        embed_code = data.get("embed_code", None)
        is_published = data.get("is_published", None)
        published_at = data.get("published_at", None)

        # 1) Must have at least one of url or embed_code
        if not video_url and not embed_code:
            raise serializers.ValidationError(
                {"url": ["Provide either a URL or embed code for the video."]}
            )

        # 2) If publishing but no timestamp, fill in now
        if is_published:
            if not published_at:
                data["published_at"] = timezone.now()
    ...
    def get_thumbnail_url(self, obj):
        """
        Build an absolute URI to the thumbnail if set; otherwise return None.
        """
        request = self.context.get("request")
    ...
    def create(self, validated_data):
        """
        Create a new Video instance. Model.save() logic will handle slug generation
    ...
    def update(self, instance, validated_data):
        """
        Apply partial/full updates. If toggling is_published from False → True
        and published_at not in validated_data, set published_at to now.
        """
        ...
        # If moving from draft to published without explicit timestamp, set now
